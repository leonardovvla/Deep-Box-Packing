...
def create_dataset( blocks_num, train_size, valid_size, obj_dim, size_range, seed=None):
    blocks_num = int(blocks_num)
    if seed is None:
        seed = np.random.randint(123456789)
    np.random.seed(seed)

    train_dir = './data/rand_3d/pack-train-' + str(blocks_num) + '-' + str(train_size) + '-' + str(size_range[0]) + '-' + str(size_range[1]) + '/'
    valid_dir = './data/rand_3d/pack-valid-' + str(blocks_num) + '-' + str(valid_size) + '-' + str(size_range[0]) + '-' + str(size_range[1]) + '/'
    ...
    if not os.path.exists(train_dir + 'blocks.txt'):
        generate_data(train_dir, obj_dim, train_size)
    if not os.path.exists(valid_dir + 'blocks.txt'):
        generate_data(valid_dir, obj_dim, valid_size)
    return train_dir, valid_dir
